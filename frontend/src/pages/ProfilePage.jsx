...
const BUILDS = [
  { icon: BuildIcons.web,    title: 'Landing Page with AI',   unlocked: (score) => score >= 30 },
  { icon: BuildIcons.bot,    title: 'Chatbot Interface',       unlocked: (score) => score >= 50 },
  { icon: BuildIcons.api,    title: 'REST API (AI-assisted)',  unlocked: (score) => score >= 65 },
  { icon: BuildIcons.db,     title: 'Database-backed App',     unlocked: (score) => score >= 80 },
  { icon: BuildIcons.deploy, title: 'Full-Stack Deploy',       unlocked: (score) => score >= 95 },
]
...
const PATH_ICONS = {
  'vibe-web':    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round"><circle cx="12" cy="12" r="10"/><path d="M2 12h20M12 2a15.3 15.3 0 014 10 15.3 15.3 0 01-4 10 15.3 15.3 0 01-4-10 15.3 15.3 0 014-10z"/></svg>,
  'vibe-coding': <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round"><polyline points="16 18 22 12 16 6"/><polyline points="8 6 2 12 8 18"/></svg>,
  'git-github':  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round"><circle cx="18" cy="18" r="3"/><circle cx="6" cy="6" r="3"/><path d="M13 6h3a2 2 0 012 2v7"/><line x1="6" y1="9" x2="6" y2="21"/></svg>,
  'python-basics': <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round"><polyline points="4 17 10 11 4 5"/><line x1="12" y1="19" x2="20" y2="19"/></svg>,
  'apis':        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round"><path d="M8 9l3 3-3 3"/><line x1="13" y1="15" x2="16" y2="15"/><rect x="3" y="3" width="18" height="18" rx="2"/></svg>,
  'sql-basics':  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round"><ellipse cx="12" cy="5" rx="9" ry="3"/><path d="M21 12c0 1.66-4 3-9 3s-9-1.34-9-3"/><path d="M3 5v14c0 1.66 4 3 9 3s9-1.34 9-3V5"/></svg>,
}
    ...
    <div className={styles.page}>
      ...
      <div className={styles.profileCard + ' animate-fade-in'}>
        <div className={styles.avatarLg}>
          <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="var(--bg-base)" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
      ...
      <div className={styles.section + ' animate-fade-in'} style={{ animationDelay: '160ms' }}>
        ...
        <p className={styles.buildsSub}>Things you can now build with AI based on your MEK score.</p>
        <div className={styles.buildsList}>
          {BUILDS.map((b, i) => {
            const isUnlocked = b.unlocked(user.mekScore)
            return (
              <div key={i} className={`${styles.buildRow} ${isUnlocked ? styles.buildUnlocked : styles.buildLocked}`}>
